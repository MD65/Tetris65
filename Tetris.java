...
public class Tetris extends JApplet{
	...
	class TetrisPanel extends JPanel implements ActionListener {
		...
		public void paintComponent(Graphics g){
			...
			if(landed){
				rowCheck();
				currentPieces = new Pieces(randomPieces());
				landed = false;
				score+=25;
				end = gameOver();
			}
	...
	private class KeyHandler implements KeyListener{
		...
		public void keyPressed(KeyEvent event) {
		    ...
		    case KeyEvent.VK_LEFT:
		    	reset();
		    	if(!isStopped || !end){
			    	for(int i = 0; i<currentPieces.a.length; i++){
			    		for(int j = 0; j<currentPieces.a[0].length-1;j++){
			    			if(currentPieces.a[i][j] > 0){
			    				temp[i-1][j] = currentPieces.a[i][j];
			    			}
			    			if (currentPieces.a[i][j] > 0 && grid[i-1][j] != 0) ok = false;
			    		}
			    	}
			    	if (ok)
			    		currentPieces.a = copy(temp);
		    	}
		    	break;
		    ...
		    case KeyEvent.VK_RIGHT:
		    	reset();
		    	if(!isStopped || !end){
		    		for(int i = 0; i<currentPieces.a.length-1; i++){
		    			for(int j = 0; j<currentPieces.a[0].length-1;j++){
		    				if(currentPieces.a[i][j] > 0 && grid[i+1][j]==0){
		    					temp[i+1][j] = currentPieces.a[i][j];
		    				} 
		    				if (currentPieces.a[i][j] > 0 && grid[i+1][j] != 0) ok = false;
		    ...
		    case KeyEvent.VK_DOWN:
		    	...
		    	if(!isStopped ||!end){
		    		for(int i = 0; i<currentPieces.a.length; i++){
		    			for(int j = 0; j<currentPieces.a[0].length-1;j++){
		    				if(currentPieces.a[i][j] > 0 && grid[i][j+1]==0){
		    					temp[i][j+1] = currentPieces.a[i][j];
		    				}
		    			}
		    		}	
		    		currentPieces.a = copy(temp);
		    ...
		    case KeyEvent.VK_UP:
		    	if(!isStopped || !end){
		    		currentPieces.getLocation();
		    		currentPieces.rotate(letter);
		    	}
		    	break;	
